...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/1776CASHParams"
    else
        PARAMS_DIR="$HOME/.1776CASH-params"
    fi
fi
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/1776cash/$filename"
        if ! [ -f "$filename" ]; then
...
function main() {
    ...
    1776CASH - install-params.sh

    

    This script will install the 1776CASH zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        README_PATH="$PARAMS_DIR/README"
                cat >> "$README_PATH" <<EOF
        This directory stores common 1776CASH zkSNARK parameters. Note that it is
