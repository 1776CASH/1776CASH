...
#if defined(NDEBUG)
#error "1776CASH cannot be compiled without assertions."
#endif
...
{
    // Custom monetary policy is applied to mainnet.
    // Keep testnet/regtest on legacy/dev schedules expected by testing workflows.
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        // - Total supply cap: Params().GetConsensus().nMaxMoneyOut (e.g. 177,600,000)
        // - Premine: chain-defined mainnet launch allocation at height 1
        // - Subsidy: fixed chain-defined mainnet block reward until cap is reached, then 0
        //
    ...
    // Default 1776CASH regtest/testnet schedule (kept for development/testing).
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k coins.
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        if ((block.nTime == (uint32_t) consensus.nPivxBadBlockTime) &&
            ...
            // accept 1776CASH block minted with incorrect proof of work threshold
            return true;
        }
