...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for 1776CASH cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: 1776CASH address and we have full public key:
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "1776cash-cli and the GUI. There is no server-side difference."
        );
