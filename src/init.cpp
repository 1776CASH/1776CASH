 ...
 */
const char * const PIVX_PID_FILENAME = "1776cash.pid";
...
void Shutdown()
{
    ...
    util::ThreadRename("1776cash-shutoff");
...
{
    util::ThreadRename("1776cash-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that 1776CASH is running in a usable environment with all
...
{
    // Make sure only a single 1776CASH process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile because if 1776CASH is started in the future " /* Continued */
                  "from a different location. It will be unable to locate the current data files. There could " /* Continued */
                  "also be data loss if 1776CASH is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //1776CASH specific: zerocoin and spork DB's
                zerocoinDB.reset();
                ...
                // 1776CASH: load previous sessions sporks if we have them.
