...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    ...
    //
    // 1776cash starts with a digit and is not a valid URI scheme for QUrl parsing, so map it
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "1776CASH (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "1776CASH (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "1776CASH.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for 1776CASH*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=1776CASH (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=1776CASH (regtest)\n";
        else
            optionFile << "Name=1776CASH\n";
